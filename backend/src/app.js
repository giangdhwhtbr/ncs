...
// Log all routes and methods
const logRoutes = (router, basePath = '') => {
  router.stack.forEach(middleware => {
    if (middleware.route) { // Route registered directly on the router
      ...
      const newBasePath = `${basePath}${middleware.regexp.source
        .replace('^\\', '')
        .replace('\\/?(?=\\/|$)', '')}`;
      logRoutes(middleware.handle, newBasePath);
    }
  });
}

logRoutes(routes, '/api/v1');
